...
def key_generate(key):
    ...
    for x in range(16):
        l = leftRotate(l, rotations[x])
        r = leftRotate(r, rotations[x])
...
def s_box(text):
    ...
    for i in range(8):
        bits = text[i]
        r = s_matrixs[i][bin_to_int(
            [bits[0], bits[5], bits[1], bits[2], bits[3], bits[4]])]
        for bit in int_to_bin(r, 4):
            res.append(bit)
...
def feistel(text, key):
    return permutation(s_box(bin_xor(expand(text), key)))


def des_round(l, r, key):
    return r, bin_xor(l, feistel(r, key))


def hex_to_bin(input: str) -> int:

    try:
        input = format(int(input[2:], 16), '064b')
    except TypeError:
        print("Input must be a hex")
        sys.exit()
...
def des_decryption(ciphertext, key) -> str:
    key = hex_to_bin(key)
...
def get_parser():
    parser = argparse.ArgumentParser(
        prog="decrypt.py",
        description=
        "decrypt ciphertext by the method and key entered by the user",
    )
...
def main():
    ...
    print(hex(des_decryption(args.input, args.key)))
